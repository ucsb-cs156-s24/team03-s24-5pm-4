...
describe("UCSBDiningCommonsMenuItemsEditPage tests", () => {
    describe("when backend doesn't return data", () => {
        ...
        test("renders header but table is not present", async () => {
            ...
            await screen.findByText("Edit UCSB Dining Commons Menu Items");
    ...
    describe("tests where backend is working normally", () => {
        ...
        test("Is populated with the data provided", async () => {
            ...
            expect(mockToast).toBeCalledWith("UCSBDiningCommonsMenuItems Updated - id: 17 name: Pizza");
        ...
        test("Changes when you click Update", async () => {
            ...
            const idField = screen.getByTestId("UCSBDiningCommonsMenuItemsForm-id");
            const diningCommonsField = screen.getByTestId("UCSBDiningCommonsMenuItemsForm-diningCommonsCode");
            const nameField = screen.getByTestId("UCSBDiningCommonsMenuItemsForm-name");
            const stationField = screen.getByTestId("UCSBDiningCommonsMenuItemsForm-station");
            const submitButton = screen.getByTestId("UCSBDiningCommonsMenuItemsForm-submit");

            expect(idField).toHaveValue("17");
            expect(diningCommonsField).toHaveValue("De La Guerra");
            expect(nameField).toHaveValue("Tacos");
            expect(stationField).toHaveValue("Blue Plate");
            expect(submitButton).toBeInTheDocument();

            fireEvent.change(diningCommonsField, { target: { value: 'Carillo' } });
            fireEvent.change(nameField, { target: { value: 'Pizza' } })
            fireEvent.change(stationField, { target: { value: 'Italian' } })

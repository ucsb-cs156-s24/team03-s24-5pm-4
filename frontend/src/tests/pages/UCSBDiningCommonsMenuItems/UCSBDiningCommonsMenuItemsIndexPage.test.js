...
describe("UCSBDiningCommonsMenuItemsIndexPage tests", () => {
    ...
    test("Renders with Create Button for admins", async () => {
        setupAdminUser();
    ...
    test("renders three menu items correctly for regular user", async () => {
        ...
        const diningCommonsCode = screen.getByText("2");
        expect(diningCommonsCode).toBeInTheDocument();

        const name = screen.getByText("Beef Stroganoff");
        expect(name).toBeInTheDocument();

        const station = screen.getByText("Blue Plate Special");
    ...
    test("what happens when you click delete, admin", async () => {
        ...
        axiosMock.onGet("/api/ucsbdiningcommonsmenuitems/all").reply(200, ucsbDiningCommonsMenuItemsFixtures.threeDiningCommonsMenuItems);
        axiosMock.onDelete("/api/ucsbdiningcommonsmenuitems").reply(200, "Menu Item with id 2 was deleted");
        ...
        await waitFor(() => { expect(mockToast).toBeCalledWith("Menu Item with id 2 was deleted") });

...
public class UCSBDiningCommonsMenuItemsIT {
    ...
    public void an_admin_user_can_post_a_new_ucsbdiningcommonsmenuitem() throws Exception {
            ...
            UCSBDiningCommonsMenuItems menuItemOne = UCSBDiningCommonsMenuItems.builder()
            .id(2)
            .name("PineapplePizza")
            .station("Hawaiian")
            .diningCommonsCode("Carillo")
            ...
            ucsbDiningCommonsMenuItemsRepository.save(menuItemOne);

            // act
            MvcResult response = mockMvc.perform(
                            post("/api/ucsbdiningcommonsmenuitems/post?name=PineapplePizza&station=Hawaiian&diningCommonsCode=Carillo")
                                            .with(csrf()))
                            .andExpect(status().isOk()).andReturn();

            // assert
            String expectedJson = mapper.writeValueAsString(menuItemOne);
            String responseString = response.getResponse().getContentAsString();
            assertEquals(expectedJson, responseString);
    }
